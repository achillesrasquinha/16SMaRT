...
def preprocess_seqs(data_dir = None, *args, **kwargs):
    ...
    target_files = [{
        "source": "merged.unique.good.precluster.pick.pick.phylip.tre",
        "target": osp.join(data_dir, "output.tre")
    }, {
        "source": "merged.unique.good.precluster.pick.pick.opti_mcc.%s.cons.taxonomy" % cutoff_level,
        "target": osp.join(data_dir, "output.taxonomy"),
    }, {
        "source": "merged.unique.good.precluster.pick.count_table",
        "target": osp.join(data_dir, "output.count_table")
    }, {
        "source": "merged.unique.good.precluster.pick.pick.opti_mcc.list",
